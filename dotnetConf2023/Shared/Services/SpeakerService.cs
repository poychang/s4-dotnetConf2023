        ...
        public IEnumerable<SpeakerModel> GetSpeakers()
        {
            var speakers = new List<SpeakerModel> {
                ...
                new SpeakerModel {
                    ...
                    TopicOutline = "身為開發人員的你，是否碰過: 會的語言與開發工具越來越多，但是面對越來越大規模的開發，你總是覺得不知道該如何動手設計? 即使看了很多大廠的設計架構，你也不知道是否照抄就能做得出來，並且滿足老闆的期待?\r\n\r\n恭喜你已經成長到這階段，只要能跨過去就能開始接觸大型系統的設計了。架構設計其實不難，困難的是你要能驗證想法是可行的。這個場次，我就是要介紹這技巧: 系統模型的建立與降級驗證。\r\n\r\n去年我同樣在 .NET Conf 聊到 API First, 背後談的其實是從商業模型轉換成 API 介面的設計技巧；而今年，我想延續這主題，談談介面背後的整個系統架構該如何設計與驗證。其中降級的技巧我認為是最關鍵的一環，他能讓你將需要多台設備部署上線才能驗證的系統，簡化成單一專案 (我用 C# 示範) 就能驗證可行性的技巧。\r\n\r\n降級, 就是將架構設計常用的元件, 通通都 \"降級\" 成 程式語言 (C#) 能表達的相同概念元件, 降級後你就能用簡單的程式碼，快速模擬你的系統將來執行起來會怎麼運作。舉個例子，若你打算實際系統要應用 Message Bus 來進行 Pub / Sub 的設計，降級成 C# 的 event, 你就能快速驗證事件的設計是否可行了；再舉一個例子，Message Queue, 你只要用 C# 的 BlockingCollection<T> 就可以。資料庫也同樣，受惠於 C# 的 LINQ，你只需要短短幾行 code, 就能用 collection + LINQ 模擬資料庫設計。掌握這技巧，架構師就能快速地驗證設計，並且將這些設計規格，拆分給不同的負責團隊進行實際的開發。\r\n\r\n我在今年的 DevOpsDays 2023 Keynotes, 談了同樣的主題，不過我在那場只談了前半段，而後半段的降級技巧，正好搭配 C# 的實做範例，我選擇了在 .NET Conf 的場合來跟大家介紹這內容，歡迎所有有興趣的朋友們來聽我這場演講，也歡迎各位在會後跟我當面聊聊你的心得與想法。",
                    Room = "",
                    Track = "",
